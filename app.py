...
def calcular_cuotas(monto, tasa_anual, plazo_meses, frecuencia, tipo_cuota, incluir_seguro, porcentaje_seguro):
    ...
    if pagos_por_año == 0:  # Al vencimiento
        ...
        lines = [f"{'Pago':<5} {'Cuota':<10} {'Interés':<10} {'Abono':<10} {'Seguro':<10} {'Saldo':<10}"]
        lines.append(f"{1:<5} {cuota_total:<10.2f} {interes:<10.2f} {abono:<10.2f} {seguro:<10.2f} {0.0:<10.2f}")
        return "\n".join(lines)
    ...
    if tipo_cuota == 'Nivelada':
        for i in range(1, n_pagos + 1):
            ...
            lines.append(f"{i:<5} {cuota_total:<10.2f} {interes:<10.2f} {abono:<10.2f} {seguro_aplicado:<10.2f} {saldo:<10.2f}")
    else:
        abono_fijo = monto / n_pagos
        for i in range(1, n_pagos + 1):
            interes = saldo * tasa_periodo
            ...
            seguro_aplicado = seguro_unitario if (incluir_seguro == 'Sí' and i <= cuotas_con_seguro) else 0
            cuota_total = cuota_base + seguro_aplicado
            lines.append(f"{i:<5} {cuota_total:<10.2f} {interes:<10.2f} {abono_fijo:<10.2f} {seguro_aplicado:<10.2f} {saldo:<10.2f}")
